...
class UNet(nn.Module):
    def __init__(
        ...
        #First layer in the list below is E_T, the second one is E_tau (for the intermediate iterates) 
        down_layers   = [conv2d(in_channel * (fold ** 2), channel, 3, padding=1),conv2d(in_channel * (fold ** 2), channel, 3, padding=1)]
        ...
        #D_tau
        self.out_inter = nn.Sequential(
            nn.GroupNorm(32, in_channel),
            Swish(),
            conv2d(in_channel, 3 * (fold ** 2), 3, padding=1, scale=1e-10),
        )

    def forward(self, input, time, type):

        #For the usual iterates
        if type == 0:
        ...
        #For intermediate iterates
            ...
            for layer in self.down:
                ...
                #E_tau is the 1st layer in the self.down sequential list
                if (count_inter != 0):
            ...
            # Intermediate iterates do not utilize the MID block (2 resnet blocks)
            # for layer in self.mid:
            #     out = layer(out, time_embed)
