...
class DDP(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        #here we sample a timestep between 0.5 - 1, which we use for the final iterates.
        #the timestep for intermediate iterate will be equal to time/2 and is done directly before the forward pass.
        time   = ((((0.5 - 1) * torch.rand(img.shape[0])) + 1) * 1000).type(torch.int64).to(img.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_gpu", type=int, default=3, help="Number of available GPUs.")
