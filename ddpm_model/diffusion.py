# Code base inherited from https://github.com/hmdolatabadi/denoising_diffusion
...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, model, x, t, clip_denoised, return_pred_x0):
        ...
        #Using the usual layers for sampling, by default
        model_output = model(x, t, 0)

        #Uncomment the below lines to generate samples from Intermediate layers
        # t = torch.div(t, 2).type(torch.int64).to(t.device)
        # model_output = model(x, t, 1)
    ...
    def training_losses(self, model, x_0, t, noise=None):
        ...
        time_by2 = torch.div(t, 2).type(torch.int64).to(t.device)

        x_t = self.q_sample(x_0=x_0, t=t, noise=noise)

        x_tby2 = self.q_sample(x_0=x_0, t=time_by2, noise=noise)
        ...
        if self.loss_type == 'kl':
            ...
            # 0 means normal interate
            model_output = model(x_t, t, 0)


            #1 means intermediate iterate
            model_output_inter = model(x_tby2, time_by2, 1)

            #Our loss function, here alpha is 0.5
            losses  = 0.5 * (torch.mean((target - model_output).view(x_0.shape[0], -1)**2, dim=1)) + 0.5 * (torch.mean((target - model_output_inter).view(x_0.shape[0], -1)**2, dim=1))
